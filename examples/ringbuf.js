(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    ...
    factory(mod.exports, global.babelRuntimeHelpersClassCallCheck, global.babelRuntimeHelpersCreateClass);
    global.ringbuf = mod.exports;
  }
  ...
  var RingBuffer = /*#__PURE__*/function () {
    (0, _createClass2["default"])(RingBuffer, null, [{
      key: "getStorageForCapacity",
      value: function getStorageForCapacity(capacity, type) {
    ...
    (0, _createClass2["default"])(RingBuffer, [{
      ...
      value: function push(elements) {
        ...
        var to_write = Math.min(this._available_write(rd, wr), elements.length);
        var first_part = Math.min(this._storage_capacity() - wr, to_write);
        var second_part = to_write - first_part;

        this._copy(elements, 0, this.storage, wr, first_part);

        this._copy(elements, first_part, this.storage, 0, second_part); // publish the enqueued data to the other side
      ...
      value: function pop(elements) {
        ...
        var to_read = Math.min(this._available_read(rd, wr), elements.length);
        var first_part = Math.min(this._storage_capacity() - rd, elements.length);
      ...
      key: "available_write",
      value: function available_write() {
        var rd = Atomics.load(this.read_ptr, 0);
        var wr = Atomics.load(this.write_ptr, 0);
      ...
      key: "_available_read",
      value: function _available_read(rd, wr) {
        if (wr > rd) {
          return wr - rd;
        } else {
      ...
      value: function _available_write(rd, wr) {
        var rv = rd - wr - 1;
      ...
      value: function _storage_capacity() {
      ...
      key: "_copy",
      value: function _copy(input, offset_input, output, offset_output, size) {
